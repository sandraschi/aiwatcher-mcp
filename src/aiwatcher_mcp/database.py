...
async def get_recent_items(
    ...
    feed_type_clause = "AND f.feed_type = ? " if feed_type else ""
    exclude_clause = ""
    ...
    if exclude_feed_ids:
        placeholders = ",".join("?" * len(exclude_feed_ids))
        exclude_clause = f"AND i.feed_id NOT IN ({placeholders}) "
        params.extend(exclude_feed_ids)
...
async def get_cached_digest(hours: int, ttl_minutes: int) -> dict[str, Any] | None:
    ...
    return {
        "subject": f"AIWatcher Daily Digest - {datetime.now(UTC).strftime('%Y-%m-%d')} (cached, {count} items)",
