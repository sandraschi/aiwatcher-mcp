...
const TOOL_DOCS: Record<string, string> = {
	poll_feeds:
		"Poll all enabled RSS/Atom feeds for new items. Manually trigger outside the scheduled 30-minute interval.",
	distill_pending:
		"Score unprocessed items with Claude (claude-sonnet-4). Each item gets relevance (0–10), urgency (0–10), a Sandra-voice summary, and tags.",
	check_alerts:
		"Find items >= ALERT_THRESHOLD (default 8.5) and fire robofang Council POST + speechops TTS. Falls back to Windows SAPI5.",
	generate_digest:
		"Build a full HTML + plain-text digest from scored items in the last N hours.",
	send_digest_now:
		"Force-send the digest email to Sandra and Steve outside the 07:00 UTC schedule.",
	get_top_items:
		"Return top N items sorted by urgency_score DESC for a given time window.",
	get_feeds_list:
		"List all configured feeds with enabled status, type, and last fetch timestamp.",
	add_feed:
		'Add a new RSS/Atom source. name + url required; feed_type defaults to "rss".',
	show_dashboard_card:
		"Prefab UI — render a stats card in Claude Desktop. Plain-text fallback for other hosts.",
};
...
export function ToolsPage() {
	...
	const run = useMutation({
		mutationFn: async (tool: string) => {
			...
			if (!endpoint)
				throw new Error(
					"No safe preview endpoint for this tool — use the Dashboard or MCP client.",
				);
