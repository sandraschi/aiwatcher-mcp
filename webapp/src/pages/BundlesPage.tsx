...
import { BundleHealthPanel } from "../components/BundleHealthPanel";
...
export function BundlesPage() {
  ...
  useEffect(() => {
    if (bundles.length > 0 && selectedBundleId === null) {
      const preferred =
        bundles.find((b: { name: string }) => b.name === "IDE Host Signal") ??
        bundles[0];
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm" style={{ color: "var(--text-muted)" }}>
            Topic-scored feeds with health metrics — start with{" "}
            <strong>IDE Host Signal</strong> for Cursor/host buzz.
          </p>
        </div>
        <button
...
function BundleView({ bundleId }: { bundleId: number }) {
  ...
  return (
    <div className="space-y-4">
      <BundleHealthPanel bundleId={bundleId} />
                ...
                <div className="space-y-1.5 max-h-48 overflow-y-auto pr-1">
                  {result.suggested_feeds.map((s: any, i: number) => (
                    <SuggestedSource
                      ...
                      bundleId={result.id}
                      onAdded={() => {
                        wizardQueryClient.invalidateQueries({
                          queryKey: ["bundles"],
                        });
                      }}
                    />
