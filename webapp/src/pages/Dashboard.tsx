...
export function Dashboard() {
    ...
    const check = async () => {
      ...
      if (!cancelled) {
        attemptRef.current = Math.min(
          ++attemptRef.current,
          HEALTH_BACKOFF.length - 1,
        );
        timerRef.current = setTimeout(
          check,
          HEALTH_BACKOFF[attemptRef.current] * 1000,
        );
      }
    };
    check();
  ...
  const restartBackend = useCallback(async () => {
    ...
    try {
      ...
      /* not in Tauri — HTTP poll updates status */
    }
    setRestarting(false);
  }, []);
    ...
    <div data-testid="dashboard" className="space-y-6 max-w-6xl">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div
          ...
          className="col-span-full flex items-center gap-2 text-xs text-muted-foreground mb-2"
        >
          <div
