        ...
        {expanded && (
          ...
          >
            <ul className="divide-y" style={{ borderColor: "var(--border)" }}>
              {alerts.map((alert) => (
                <li
                  key={`${alert.source}-${alert.code}-${alert.message}`}
                  className="px-4 py-3"
                >
            ...
            >
              Auto-refresh every 15s · threshold {data?.stale_hours ?? 48}h
              stale · try <strong>Poll Feeds</strong> if arXiv rows are empty
            </div>
          </motion.div>
        )}
      </AnimatePresence>
